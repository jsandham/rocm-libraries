...
class LayernormBackwardOperationDescriptor
    : public HipdnnBackendDescriptorImpl<LayernormBackwardOperationDescriptor>,
...
public:
    ...
    const hipdnn_flatbuffers_sdk::data_objects::LayernormBackwardAttributesT& getData() const
    {
        return _data;
    }
    ...
    // Creates a finalized LayernormBackwardOperationDescriptor directly from a FlatBuffer NodeT.
    // Casts nodeT.attributes to LayernormBackwardAttributes internally, then directly assigns
    // the data struct, looks up tensor descriptors from the tensor map, and calls finalize().
    static std::shared_ptr<LayernormBackwardOperationDescriptor>
...
private:
    hipdnn_flatbuffers_sdk::data_objects::LayernormBackwardAttributesT _data;
